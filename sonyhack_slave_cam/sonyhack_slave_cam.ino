...
static uint8_t const label[6] = {0, 1, 2, 3, 4, 5};
...
void CamCB(CamImage img) {
  ...
  if( c == a){
    ...
    if(c == 5 && n > 5){
      ...
      delay(1000);
      Serial.println("name: daisuke");
      Serial.println("face width = 140 mm");
      Serial.println("信号出力します");
      n = 0;
    }
  }else{
    n = 0;
...
void setup() {   
  ...
  File nnbfile = theSD.open("network2.nnb");
...
void loop() {
 /* //Serial.println(Ms[index - 1]);
  if(index == 1){
    Serial.print("fuyuki");
    
  }else if(index == 2){
    Serial.print("daisuke");
